...
class Endpoint:
	...
	def __init__(self, red: point_t, blue: point_t, dir: point_t):
		...
		stub_min = mul(len_red, self.dir)
		stub_max = mul(len_blue, self.dir)

		self.stub_red = adv(stub_max, red)
		self.stub_blue = adv(stub_min, blue)
...
class Connector:
	...
	def attach(self, red: point_t, blue: point_t, dir: point_t):
		endpoint = Endpoint(red=red, blue=blue, dir=dir)
		self.endpoints.append(endpoint)
		

	def paths(self):
		
		if self.target is None and len(self.endpoints) < 2:
			return

		offs = self.endpoints[0].sign * Config.link_width_cm / 2

		if self.target:
			self.path.append(self.endpoints[0].mid)
			self.path.append(self.endpoints[0].stub_mid)
			self.path.append(self.target)
			self.red_path = offset(self.path, offs)
			self.blue_path = offset(self.path, -offs)
			self.red_path[0] = self.endpoints[0].red
			self.blue_path[0] = self.endpoints[0].blue
			return

		self.path.append(self.endpoints[0].mid)
		self.path.append(self.endpoints[0].stub_mid)
		self.path.append(self.endpoints[1].stub_mid)
		self.path.append(self.endpoints[1].mid)
		...
		self.red_path[0] = self.endpoints[0].red
		self.blue_path[0] = self.endpoints[0].blue
		self.red_path[-1] = self.endpoints[1].red
		self.blue_path[-1] = self.endpoints[1].blue
